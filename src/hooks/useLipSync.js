...
export function useLipSync({ scene, analyser, getVolume = null }) {
    ...
    useFrame((state, delta) => {
        ...
        const fallbackVolume = typeof getVolume === 'function' ? Math.max(0, Math.min(1, getVolume())) : 0;
        ...
        // Bins ~14 to ~37
        const rawAA = Math.max(getTargetValue(14, 40), fallbackVolume > 0.03 ? Math.min(0.6, fallbackVolume * 1.6) : 0);
